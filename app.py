...
def like_warble(message_id):
    ...
    if message.user_id == g.user.id:
        ...
        return redirect(f"/warbles/{message.id}")
...
def liked_warbles(user_id):
    """Show the warbles liked by a user."""
    user = User.query.get_or_404(user_id)
    liked_warbles = user.likes

    return render_template('liked_warbles.html', user=user, liked_warbles=liked_warbles)